...
function CustomSelect({ title, options, selectOption, field, element, header,active }) {
    ...
    return (
        <div onClick={toggleClass} className={`${active===false ? "pointer-events-none border-0" : null } relative transition-all duration-500 w-full h-[40px] overflow-hidden ${isActive ? "active-select" : ""}`}>
            <div className={`relative h-[40px] ${active===false ? "bg-[#383838]" : "bg-[#1A1A1A]" }  flex justify-between rounded-[5px]  p-[5px]`}>
            ...
            <div className='menu-container min-[552px]:mt-[10px]'>
                <div className='mobile flex flex-col'>
                    <div className='flex flex-col items-center justify-center gap-[10px]  h-auto bg-[#1F1F1F] rounded-t-[10px] pt-[10px] pb-[15px]'>
                        ...
                        {element === "input" ? <input type="text" placeholder="Axtar..." className='outline-none w-[95%] h-[40px] min-[552px]:w-[98%] rounded-[7px] p-[5px] text-[#9CA3AF] bg-[#151515]' onClick={handleInputClick}></input> : null}
                    </div>
